...
type PornstarsClientProps = {
  data: PornstarsData;
  inter: { className: string };
};

export default function PornstarsClient({ data, inter }: PornstarsClientProps) {
  const [sortField, setSortField] = useState<"name" | "count" | "recent">(
    "name"
  );
  const [sortDirection, setSortDirection] = useState<"asc" | "desc">("asc");

  const sortedPornstars = useMemo(() => {
    const sorted = [...data.pageLinks].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "count":
          ...
          break;
        case "recent":
          comparison = (a.recentCount || 0) - (b.recentCount || 0);
          break;
      }
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="w-full">
            ...
            {data.pageLinks && data.pageLinks.length > 0 ? (
              <div className="mb-8">
                <div className="bg-[#0d0d0b] border border-[#1f1e1d] rounded-lg p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {sortedPornstars.map((link, index) => (
                      <Link
                        key={index}
                        href={link.url}
              ...
              <div className={`text-center text-gray-400 ${inter.className}`}>
                <p>No pornstars available at the moment.</p>
              </div>
            )}
          </div>
        </div>
